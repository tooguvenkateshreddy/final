...
<body>
	...
	<sql:query dataSource="${db}" var="result">
       select * from product p,inventory i where p.pid=i.pid
         </sql:query>
	...
	<div class="container-fluid">
		<div class="row">
			...
			<div class="col-sm-11 col-md-8 col-lg-10">
				<div class="container">
					<div class="btn-group b1" style="width: 80%">
						<a href="products.jsp" class="btn btn-lg btn-outline-secondary">PRODUCTS</a>
						<a href="#" class="btn active btn-lg btn-outline-secondary">INVENTORY</a>
					</div>
					<div class="form-inline d3">
						...
						<select class="form-rounded col-sm-2 ml-sm-2 mr-sm-2"
							...
							<option>3</option>
							<option>4</option>
						</select> <a href="#"> <input type="submit"
					...
					<div class="container">
						<table class="table t1 ">
							...
							<form action="update?s1=${row.pid}" method="post">
								<tr>
									<td><input type="checkbox" class="r1 mr-sm-2" /> <c:out
											value="${row.name}" /></td>
									<td><c:out value="${row.pid}" /></td>
									<td class="tr1"><c:out value="${row.instock}" /></td>
									<td class="tr1"><c:out value="${row.req}" /></td>
									<td><c:out value="${row.buffer}" /></td>
									<td><c:out value="${row.pending}" /></td>
									<td><c:out value="${row.rating}" /></td>
									<td>
									
										<button type="button"
											class="form-rounded  br1 ml-sm-2 mr-sm-2" data-toggle="modal"
											data-target="#myModal">Manage</button>
										<div class="modal" id="myModal" action="#">
											<div class="modal-dialog">
												<div class="modal-content">
													<div class="modal-body">
														<h2 class="modal-title" align="left">Update Stock</h2>

														Enter Stock value to be updated: <input type="text"
															name="s2" placeholder="Enter Stock" />
													</div>
													<div class="model-footer">
														<input type="submit" value="Done"
															class="btn btn-secondary"
															style="border-radius: 1rem; border: 0px solid; margin: 12px; width: 125px;">
														<button type="button" class="btn d4" data-dismiss="modal"
															style="border-radius: 1rem; background-color: #E5E7EA; border: 0px solid; width: 125px;">Close</button>
													</div>
												</div>
											</div>
										</div>
